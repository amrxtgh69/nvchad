# My NvChad Configuration (AI-Generated)
...
| [peek.nvim](https://github.com/toppair/peek.nvim) | Markdown preview (GUI) |
...
### Markdown Preview (Terminal-Only with Images)
| Keybinding | Action |
|------------|--------|
| `<leader>mo` | Open markdown preview in vertical split |
| `<leader>mr` | Refresh existing markdown preview |

#### Overview
A **first-principles** implementation for rendering markdown with images **entirely in the terminal** using NvChad + Kitty + mdcat.

> **⚠️ AI-Generated Documentation**  
> This section was generated by AI assistant. Verify implementation details before use.

**Features:**
✅ 100% terminal-based - no browser, no GUI  
✅ Inline images via Kitty's graphics protocol  
✅ Side-by-side editing - source on left, preview on right  
✅ Live updates - refresh on save or manual  
✅ Zero dependencies - except mdcat itself  

**Requirements:**
- **Kitty Terminal** (required for images)
- **mdcat** installed: `cargo install mdcat` or package manager

#### Usage
1. Open markdown file: `nvim README.md`
2. Press `<leader>mo` to open preview
3. Edit source, see rendered output on right
4. Save (`:w`) for auto-refresh or `<leader>mr` manual

#### Image Support
✅ **Works:** Local PNG/JPG/GIF with relative paths  
❌ **Doesn't work:** HTTP URLs, SVG files  

Example: `![diagram](./images/arch.png)`

#### Configuration Files
```lua
-- lua/configs/mdcat.lua
local M = {}
...
#### Troubleshooting
- Images not showing? Verify `echo $TERM` contains 'kitty'
- Test manually: `mdcat yourfile.md`
- Preview opens on right side of vertical split
